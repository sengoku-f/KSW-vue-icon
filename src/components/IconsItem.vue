<template>
  ...
  <div class="flex mx-auto max-w-7xl gap-6 mt-6 mb-12 px-4 sm:px-6 lg:px-8">
    <aside
      class="sidebar flex-shrink-0 w-56 overflow-auto overscroll-auto sticky top-[84px]"
    >
      <ul class="flex flex-col gap-1">
        <li v-for="category in categories" :key="category">
          <div
            @click="changeCategory(category)"
            class="w-full px-3 py-2 rounded-lg cursor-pointer select-none text-slate-500 font-normal hover:bg-slate-50 transition-all text-sm"
...
const sortIcons = () => {
  ...
  if (showColorIcons.value === "color") {
    ...
    sortedIcons = sortedIcons.filter(
      (icon) => !icon.componentName.includes("Color")
    );
  }
...
const filteredIcons = computed(() => {
  return icons.value.filter((icon) =>
    icon.componentName.toLowerCase().includes(searchQuery.value.toLowerCase())
  );
});
</script>
